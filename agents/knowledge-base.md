...
This file contains domain knowledge about the CoachOS Booking product and codebase. For coding guidelines and rules, see [`rules/`](rules/).
...
### CoachOS Booking Event Identification

CoachOS Booking events in Google Calendar can be identified by checking if the iCalUID ends with `@CoachOS Booking` (e.g., `2GBXSdEixretciJfKVmYN8@CoachOS Booking`). This identifier is used to distinguish CoachOS Booking bookings from other calendar events for data storage and privacy purposes.
...
Workflows and webhooks are two completely separate features in CoachOS Booking with different implementations and file structures:
