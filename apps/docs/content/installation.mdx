...
There are multiple ways in which you can deploy CoachOS Booking, providing support for customers who want to implement CoachOS Booking within their existing infrastructure stack. Let's go through them one by one. You can find the instructions for deployment in our README file, which is the section you see when you scroll down in our GitHub repository, or if you've got a copy of CoachOS Booking downloaded already, you can open the file contained in the downloaded repository called `README.md`.

## Requirements

CoachOS Booking runs with pretty minimal hardware requirements by itself. The most intensive part for the software is when you actually build the software, but once it's running it's relatively lightweight.

CoachOS Booking works with a very large range of operating systems, as it only requires JavaScript execution to run. **CoachOS Booking is known to work well with Windows, Mac, Linux and BSD.** Although they do work well on all of them, for production deployments we would suggest Linux as the ideal platform. Any operating system that runs Node.js should be able to work too, but these are some of the common operating systems that we know work well.

To run CoachOS Booking, you need to install a few things. **Node.js, yarn, Git and PostgreSQL**. We use **Prisma** for database maintenance, and is one of the dependencies. We won't publish installation guides for these as they have their own resources available on the internet. If you're on Linux/BSD, all of these things should be readily available on your package manager. Your best bet is searching for something like **Debian 12 PostgreSQL**, which will give you a guide to installing and configuring PostgreSQL on Debian Linux 12.
...
```bash
git clone https://github.com/amir9078/coachos-booking.git
```

> If you are on windows, you would need to use the following command when cloning, with **admin privileges**:
>
> ```bash
> git clone -c core.symlinks=true https://github.com/amir9078/coachos-booking.git
> ```
...
to install all of the dependencies. Essentially, dependencies are just things that CoachOS Booking needs to install to be able to work.
...
At CoachOS Booking, the cron jobs are found in the following directory:
...
```bash
yarn workspace @coachos/api-v2 build
yarn workspace @coachos/api-v2 start
```
...
[![Deploy with Vercel](https://vercel.com/button)](https://vercel.com/new/clone?repository-url=https://github.com/amir9078/coachos-booking)

## Elestio

[![Deploy on Elestio](https://elest.io/images/logos/deploy-to-elestio-btn.png)](https://elest.io/open-source/CoachOS Booking)

## Other environments

CoachOS Booking effectively is just a Next.js application, so any possible solution you find online pertaining to Next.js applications should work. One example is Netlify, which is pretty similar to Vercel. It says it supports Next.js, so you can deploy CoachOS Booking on Netlify. Refer to Netlify's docs on Next.js projects for more info. Another example is on a self hosted instance people may want to configure complex reverse proxies, SSL gateways and all sorts of other stuff. We can't officially support every configuration, but for any edge case where you may want to deploy CoachOS Booking with X, just refer to X's docs on Next.js applications and you should be fine. That's it. Your new self hosted CoachOS Booking instance should now be up and running.
