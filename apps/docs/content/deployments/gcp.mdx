...
Deploying CoachOS Booking on Google Cloud Platform (GCP). In this guide, we will go over the steps to deploy CoachOS Booking on Google Cloud Platform (GCP). We will cover how to create a virtual machine, configure it, install Docker, and finally deploy the CoachOS Booking application.

## One Click Deployment

[![Run on Google Cloud](https://storage.googleapis.com/gweb-cloudblog-publish/images/run_on_google_cloud.max-300x300.png)](https://deploy.cloud.run/?git_repo=https://github.com/calcom/CoachOS Booking)
...
### 4. Deploying CoachOS Booking

Now that Docker is installed and running, you can deploy CoachOS Booking on your virtual machine.

#### Pull the Docker Image

Use the following command to pull the CoachOS Booking Docker image from Docker Hub:

```bash
docker pull calcom/CoachOS Booking
```
...
```bash
docker run -d -p 80:80 calcom/CoachOS Booking
```

This command maps port 80 on your local machine to port 80 inside the container, so you can access CoachOS Booking from outside the container.

#### Access CoachOS Booking

Open a web browser and navigate to `http://localhost`. You should now be able to access the CoachOS Booking homepage.

Congratulations! You have successfully deployed CoachOS Booking on Google Cloud Platform.
