...
This guide covers the most common issues encountered when self-hosting CoachOS Booking, along with their solutions.
...
Related issue: [#25993](https://github.com/amir9078/coachos-booking/issues/25993)
...
**Symptom:** After deploying CoachOS Booking to a server or domain, login redirects or internal links point back to `http://localhost:3000` instead of your actual domain.

**Cause:** The environment variables `NEXTAUTH_URL` and `NEXT_PUBLIC_WEBAPP_URL` are not set to your production domain. These variables tell CoachOS Booking and NextAuth where the app is hosted.
...
Related issue: [#21921](https://github.com/amir9078/coachos-booking/issues/21921)
...
**Symptom:** After running `docker compose up -d`, the `coachos-api` service fails to start or immediately exits. The web app works, but API v2 endpoints (`/api/v2/...`) return connection errors.
...
**Solution:** Add the following variables to your root `.env` file (the `docker-compose.yml` passes these to the `coachos-api` service):
...
COACHOS_ENCRYPTION_KEY=your_32_character_encryption_key
...
```bash
docker compose logs coachos-api
```
...
**Symptom:** Requests fail with SSL certificate errors when CoachOS Booking is behind a load balancer or reverse proxy that handles HTTPS termination.
...
1. Search the [CoachOS Booking GitHub Issues](https://github.com/amir9078/coachos-booking/issues) — many common problems have documented solutions in issue threads.
2. Check the [amir9078.github.io Community](https://community.amir9078.github.io) forum.
