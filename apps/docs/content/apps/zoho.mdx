...
```
<CoachOS Booking URL>/api/integrations/zohocrm/callback
```

Replace `<CoachOS Booking URL>` with your application URL.
...
10. **Integration is Ready** - Your ZohoCRM integration can now be easily added in the CoachOS Booking settings.
...
```
<CoachOS Booking URL>/api/integrations/zohocalendar/callback
```
...
5. **Save Credentials in CoachOS Booking Admin Panel** - Copy the Client ID and Client Secret to your app keys in the CoachOS Booking admin panel at:

```
<CoachOS Booking>/settings/admin/apps
```
...
```
<CoachOS Booking URL>/api/integrations/zoho-bigin/callback
```
...
7. **Complete Zoho Bigin Integration** - Your Zoho Bigin integration is now ready and can be added from the CoachOS Booking app store.
