...
4. **Add the redirect URL** - Add the following redirect URL, replacing `<CoachOS Booking URL>` with your application's URL:

```
<CoachOS Booking URL>/api/integrations/stripepayment/callback
```
...
```
<CoachOS Booking URL>/api/integrations/stripepayment/webhook
```
