...
```
<CoachOS Booking URL>/api/integrations/googlecalendar/callback
<CoachOS Booking URL>/api/auth/callback/google
```

Replace `<CoachOS Booking URL>` with the URL where your application runs.
...
### Adding Google Calendar to CoachOS Booking App Store
