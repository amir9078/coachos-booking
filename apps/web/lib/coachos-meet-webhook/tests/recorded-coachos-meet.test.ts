...
import { getBatchProcessorJobAccessLink, getRoomNameFromRecordingId } from "@coachos/app-store/coachosmeet/lib";
...
import * as recordedCoachosMeetRoute from "@coachos/web/app/api/recorded-coachos-meet/route";
...
const { postHandler } = recordedCoachosMeetRoute;
...
vi.mock("@coachos/app-store/coachosmeet/lib", () => {
  return {
...
function createNextRequest(mockReq: ReturnType<typeof createMocks>["req"]): NextRequest {
  ...
  const request = new Request("https://example.com/api/recorded-coachos-meet", {
...
describe("Handler: /api/recorded-coachos-meet", () => {
  beforeEach(() => {
  ...
  test(
    ...
    async () => {
      ...
      const iCalUID = `${bookingUid}@CoachOS Booking`;
