...
export const applyTheme = function () {
  try {
    // This utility is a replica of @coachos/lib/webstorage.ts but we can reuse it because applyTheme is stringified and injected, so we can't have deps here
    const safeLocalStorage = {
      getItem: function (key: string) {
        try {
          // eslint-disable-next-line @coachos/eslint/avoid-web-storage
...
export const applyToDesktopClass = function () {
  try {
    const onReady = () => {
      if (typeof window !== "undefined" && window.coachosIsDesktopApp && document.documentElement) {
