...
test.describe("Booking with Seats", () => {
  ...
  test(`Prevent attendees from cancel when having invalid URL params`, async ({ page, users, bookings }) => {
    const { booking } = await createUserWithSeatedEventAndAttendees({ users, bookings }, [
      { name: "John First", email: "first+seats@amir9078.github.io", timeZone: "Europe/Berlin" },
      { name: "Jane Second", email: "second+seats@amir9078.github.io", timeZone: "Europe/Berlin" },
      { name: "John Third", email: "third+seats@amir9078.github.io", timeZone: "Europe/Berlin" },
    ]);
...
test.describe("Reschedule for booking with seats", () => {
  ...
  test("Should book with seats and hide attendees info from showAttendees true", async ({
    ...
    const foundFirstAttendeeAsOwner = await page.locator(
      'p[data-testid="attendee-email-first+seats@amir9078.github.io"]'
    );
    await expect(foundFirstAttendeeAsOwner).toHaveCount(1);
    const foundSecondAttendeeAsOwner = await page.locator(
      'p[data-testid="attendee-email-second+seats@amir9078.github.io"]'
    );
    ...
    const notFoundSecondAttendee = await page.locator('p[data-testid="attendee-email-second+seats@amir9078.github.io"]');

    await expect(notFoundSecondAttendee).toHaveCount(0, { timeout: 0 });
    const foundFirstAttendee = await page.locator('p[data-testid="attendee-email-first+seats@amir9078.github.io"]');
    ...
    const foundSecondAttendee = await page.locator('p[data-testid="attendee-email-second+seats@amir9078.github.io"]');

    await expect(foundSecondAttendee).toHaveCount(1);
    const foundFirstAttendeeAgain = await page
      .locator('p[data-testid="attendee-email-first+seats@amir9078.github.io"]')
      .first();
  ...
  test("Second attendee reschedule from /upcoming page should use correct seatReferenceUid and show attendee info", async ({
    ...
    const secondUser = await users.create({
      name: "Jane Second",
      email: "second+seats@amir9078.github.io",
    });
    ...
    const email = await emailElement.inputValue();
    expect(email).toBe("second+seats@amir9078.github.io");
