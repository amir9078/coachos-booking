...
test.describe("Event Types tests", () => {
  test.describe("user", () => {
    ...
    test.describe("Different Locations Tests", () => {
      ...
      test("Can add CoachOS Meet location and book with it", async ({ page }) => {
        await gotoFirstEventType(page);

        await page.getByTestId("location-select").click();
        await page.locator(`text="CoachOS Meet (Default)"`).click();
        ...
        await expect(page.locator("[data-testid=success-page]")).toBeVisible();
        await expect(page.locator("[data-testid=where] ")).toContainText("CoachOS Meet");
      });
      ...
      test.skip("Can remove location from multiple locations that are saved", async ({ page }) => {
        ...
        // Add CoachOS Meet location
        await addAnotherLocation(page, "CoachOS Meet (Default)");
        ...
        await expect(page.locator("[data-testid=success-page]")).toBeVisible();
        await expect(page.locator("[data-testid=where]")).toHaveText(/CoachOS Meet/);
      });
