import type { MockResponse } from "@coachos/features/auth/signup/handlers/__tests__/mocks/next.mocks";
import {
  prismaMock,
  resetPrismaMock,
...
vi.mock("@coachos/lib/constants", () => ({ WEBAPP_URL: "http://localhost:3000" }));
vi.mock("@coachos/lib/tracking", () => ({ getTrackingFromCookies: vi.fn().mockReturnValue({}) }));
vi.mock("@coachos/app-store/stripepayment/lib/utils", () => ({ getPremiumMonthlyPlanPriceId: vi.fn() }));
vi.mock("@coachos/features/auth/lib/getLocaleFromRequest", () => ({
...
vi.mock("@coachos/features/watchlist/lib/telemetry", () => ({ sentrySpan: {} }));
...
vi.mock("@coachos/features/di/containers/FeatureRepository", () => ({
  getFeatureRepository: vi.fn().mockReturnValue({
    checkIfFeatureIsEnabledGlobally: vi.fn().mockResolvedValue(false),
  }),
}));
vi.mock("@coachos/features/watchlist/lib/repository/GlobalWatchlistRepository", () => {
  return {
...
vi.mock("@coachos/features/watchlist/lib/utils/normalization", () => ({
  normalizeEmail: vi.fn((e: string) => e.toLowerCase()),
}));
vi.mock("@coachos/web/lib/buildLegacyCtx", () => ({ buildLegacyRequest: vi.fn() }));
