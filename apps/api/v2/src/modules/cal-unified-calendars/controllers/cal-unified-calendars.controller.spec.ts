 ...
 * for DI token resolution) has runtime imports from @coachos/platform-libraries and
 * @coachos/platform-libraries/app-store whose transitive dependencies (prisma, DB,
 * Google APIs) cannot be resolved in Jest.
 * The service is fully mocked via useValue so these packages are never executed.
 */
jest.mock(
  "@coachos/platform-libraries/app-store",
  () => ({
...
import { GOOGLE_CALENDAR, SUCCESS_STATUS } from "@coachos/platform-constants";
