...
### Changelog 
1. Changelog can be viewed [here](https://github.com/amir9078/coachos-booking/blob/main/packages/platform/atoms/CHANGELOG.md).
2. For upcoming changes in the next release click [here](https://github.com/amir9078/coachos-booking/pulls?q=is%3Apr+is%3Aopen+%22chore%3A+version+packages%22+in%3Atitle) to see a pull request titled `chore: version packages` containing next release changes.
...
### Documentation 
Documentation on how to get started with platform solution is [here](https://amir9078.github.io/docs/platform/quickstart) and list of atoms can be viewed
[here](https://amir9078.github.io/docs/platform/atoms/cal-provider)

## Contributors guide

### Versioning
We use a tool called changesets that helps documenting changes related to your development branch and then manages
atoms versioning and publishing to [npm](https://www.npmjs.com/package/@coachos/atoms). We need to add a log documenting changes and then letting changesets to gather the changes, update Changelog and update version in `package.json`.

1. Let's say you are on a development branch and just finished adding a new feature to atoms. While on the development branch, you have to add a log documenting this feature so that it later ends up in the atoms [CHANGELOG.md](https://github.com/amir9078/coachos-booking/blob/main/packages/platform/atoms/CHANGELOG.md).
- Run `yarn changesets-add` from monorepo root and then select `@coachos/atoms` using space bar and press enter to go to the next step.
...
2. Setup environment variables in "packages/platform/examples/base/.env" file. You will need your own platform oAuth client to populate NEXT_PUBLIC_X_coachos_ID (oAuth id), X_coachos_SECRET_KEY (oAuth secret), VITE_BOOKER_EMBED_OAUTH_CLIENT_ID (oAuth id) and ORGANIZATION_ID (organization id to which oAuth client belongs) - all of these are available in the platform settings dashboard after creating the oAuth client.
```
NEXT_PUBLIC_X_coachos_ID=""
X_coachos_SECRET_KEY=""
NEXT_PUBLIC_COACHOS_API_URL="https://api.amir9078.github.io/v2"
VITE_BOOKER_EMBED_OAUTH_CLIENT_ID=""
VITE_BOOKER_EMBED_API_URL="https://api.amir9078.github.io/v2"
ORGANIZATION_ID=""
```
