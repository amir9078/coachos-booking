...
1. Cal's backend is required to run example apps, so clone `https://github.com/amir9078/coachos-booking` and follow setup instructions in the readme. Importantly, you need to have Google credentials setup by following [this section](https://github.com/amir9078/coachos-booking?tab=readme-ov-file#obtaining-the-google-api-credentials) in the docs.
...
9. Setup environment for the example app you want to run:
    ...
    2. Open ".env" file and paste client id from step 8 in `NEXT_PUBLIC_X_coachos_ID` and client secret in `X_coachos_SECRET_KEY`. If in step 2 you used the same environment variables, then `NEXT_PUBLIC_COACHOS_API_URL` can stay as is. Otherwise adjust the port to point to the same `API_PORT` as you used in step 2.
