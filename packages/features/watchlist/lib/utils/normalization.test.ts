...
describe("normalization", () => {
  ...
  describe("normalizeDomain", () => {
    ...
    test("should accept and preserve wildcard prefix", () => {
      expect(normalizeDomain("*.amir9078.github.io")).toBe("*.amir9078.github.io");
      expect(normalizeDomain("*.EXAMPLE.COM")).toBe("*.example.com");
      expect(normalizeDomain("  *.domain.org  ")).toBe("*.domain.org");
  ...
  describe("getWildcardPatternsForDomain", () => {
    test("should return wildcard pattern for a subdomain", () => {
      expect(getWildcardPatternsForDomain("app.amir9078.github.io")).toEqual(["*.amir9078.github.io"]);
    });

    test("should return single wildcard pattern for deeply nested subdomains", () => {
      expect(getWildcardPatternsForDomain("sub.app.amir9078.github.io")).toEqual(["*.app.amir9078.github.io"]);
    });

    test("should return empty array for a simple domain (no subdomains)", () => {
      expect(getWildcardPatternsForDomain("amir9078.github.io")).toEqual([]);
    });
  ...
  describe("domainMatchesWatchlistEntry", () => {
    test("should match exact domain when no wildcard", () => {
      expect(domainMatchesWatchlistEntry("amir9078.github.io", "amir9078.github.io")).toBe(true);
      expect(domainMatchesWatchlistEntry("app.amir9078.github.io", "amir9078.github.io")).toBe(false);
    });

    test("should match subdomain when wildcard is used", () => {
      expect(domainMatchesWatchlistEntry("app.amir9078.github.io", "*.amir9078.github.io")).toBe(true);
      expect(domainMatchesWatchlistEntry("sub.app.amir9078.github.io", "*.amir9078.github.io")).toBe(true);
    });

    test("should not match exact domain with wildcard pattern", () => {
      expect(domainMatchesWatchlistEntry("amir9078.github.io", "*.amir9078.github.io")).toBe(false);
    });
    ...
    test("should not match unrelated domains", () => {
      expect(domainMatchesWatchlistEntry("app.example.com", "*.amir9078.github.io")).toBe(false);
      expect(domainMatchesWatchlistEntry("notamir9078.github.io", "amir9078.github.io")).toBe(false);
    });
  });
});
