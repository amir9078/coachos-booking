...
import { WatchlistType, WatchlistAction, WatchlistSource } from "@coachos/prisma/enums";
...
describe("OrganizationBlockingService", () => {
  ...
  describe("isBlocked", () => {
    ...
    test("should NOT block subdomain when exact domain is blocked (no wildcard)", async () => {
      ...
      const result = await service.isBlocked("user@app.amir9078.github.io", ORGANIZATION_ID);

      expect(result.isBlocked).toBe(false);
      expect(mockOrgRepo.findBlockingEntriesForEmailsAndDomains).toHaveBeenCalledWith({
        emails: ["user@app.amir9078.github.io"],
        domains: ["app.amir9078.github.io", "*.amir9078.github.io"],
        organizationId: ORGANIZATION_ID,
      });
    });
    ...
    test("should block deeply nested subdomain with wildcard pattern", async () => {
      ...
      const result = await service.isBlocked("user@sub.app.amir9078.github.io", ORGANIZATION_ID);

      expect(result.isBlocked).toBe(true);
      expect(result.reason).toBe(WatchlistType.DOMAIN);
      expect(result.watchlistEntry).toEqual(wildcardEntry);
      expect(mockOrgRepo.findBlockingEntriesForEmailsAndDomains).toHaveBeenCalledWith({
        emails: ["user@sub.app.amir9078.github.io"],
        domains: ["sub.app.amir9078.github.io", "*.app.amir9078.github.io"],
        organizationId: ORGANIZATION_ID,
      });
    });
    ...
    test("should NOT block exact domain with wildcard pattern", async () => {
      ...
      const result = await service.isBlocked("user@amir9078.github.io", ORGANIZATION_ID);
