...
vi.mock("@coachos/features/auth/lib/verifyCodeUnAuthenticated", () => ({
...
vi.mock("@coachos/features/users/repositories/UserRepository", async (importOriginal) => {
  ...
  describe("User Email Verification Setting", () => {
    ...
    test("should create booking when main booker provides valid verification code", async () => {
      ...
      const { verifyCodeUnAuthenticated } = await import(
        "@coachos/features/auth/lib/verifyCodeUnAuthenticated"
      );
