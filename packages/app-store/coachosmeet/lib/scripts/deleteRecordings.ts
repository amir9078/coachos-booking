...
//    ts-node packages/app-store/coachosmeet/lib/scripts/deleteRecordings.ts
//
// 2. Delete recordings from the JSON file (shows preview first, then prompts for confirmation):
//    ts-node packages/app-store/coachosmeet/lib/scripts/deleteRecordings.ts delete
...
async function getAllRecordingsOlderThan6Months(): Promise<Recording[]> {
  const apiKey = process.env.COACHOS_MEET_API_KEY;

  if (!apiKey) {
    console.error("COACHOS_MEET_API_KEY environment variable is required");
    process.exit(1);
  }
