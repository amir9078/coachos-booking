...
export interface CoachosMeetCallData {
  type: string;
  id: string;
  password: string;
  url: string;
...
// Regions available to create CoachosMeet Rooms in.
const REGION_CODES = [
  "af-south-1",
  "ap-northeast-2",
  "ap-southeast-1",
...
function getCoachosMeetRegionFromEnv(): RoomGeo | undefined {
  if (!process?.env?.COACHOS_MEET_REGION) return;
  const isRoomGeo = (value: string): value is RoomGeo => REGION_CODES.includes(value as RoomGeo);
  function assertIsCoachosMeetRegion(value: string): asserts value is RoomGeo {
    if (!isRoomGeo(value)) {
      throw new Error(`Invalid region code: ${value}. Must be one of: ${REGION_CODES.join(", ")}`);
    }
  }
  const region = process.env.COACHOS_MEET_REGION;
  assertIsCoachosMeetRegion(region);
  return region;
}
...
export const FAKE_COACHOS_MEET_CREDENTIAL: CredentialForCalendarService & { invalid: boolean } = {
  id: 0,
...
const CoachosMeetApiAdapter = (): VideoApiAdapter => {
  ...
  const translateEvent = async (event: CalendarEvent, region?: RoomGeo) => {
    ...
    const { scale_plan: scalePlan } = await getCoachosMeetAppKeys();
  ...
  // Region on which the CoachosMeet room is created can be controlled by ENV var
  // undefined region leaves the choice to CoachosMeet
  const region = getCoachosMeetRegionFromEnv();
  return {
    ...
    createInstantCoachosMeetRoom: (endTime: string) => createInstantMeeting(endTime, region),
...
export default CoachosMeetApiAdapter;
