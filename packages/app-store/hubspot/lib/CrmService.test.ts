...
import type { CRM } from "@coachos/types/CrmService";
...
vi.mock("@coachos/features/bookings/repositories/PrismaTrackingRepository", () => ({
...
describe("HubspotCalendarService", () => {
  ...
  describe("createEvent", () => {
    ...
    it("should include custom text field with static value", async () => {
      ...
      const createCall = mockHubspotClient.crm.objects.meetings.basicApi.create.mock.calls[0][0];
      expect(createCall.properties.custom_source).toBe("CoachOS Booking Booking");
    });
