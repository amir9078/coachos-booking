...
export async function getHandler(req: NextApiRequest) {
  ...
  let encrypted;
  try {
    encrypted = symmetricEncrypt(JSON.stringify({ api_key }), process.env.COACHOS_ENCRYPTION_KEY || "");
  } catch (reason) {
    logger.error("Could not add Sendgrid app", reason);
    throw new HttpError({ statusCode: 500, message: "Invalid length - COACHOS_ENCRYPTION_KEY" });
  }
