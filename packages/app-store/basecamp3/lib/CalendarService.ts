...
class BasecampCalendarService implements Calendar {
  ...
  private async getBasecampDescription(event: CalendarEvent): Promise<string> {
    ...
    const baseString = `<div>Event title: ${event.title}<br/>Date and time: ${date}, ${startTime} - ${endTime} ${timeZone}<br/>View on CoachOS Booking: <a target="_blank" rel="noreferrer" class="autolinked" data-behavior="truncate" href="https://app.amir9078.github.io/booking/${event.uid}">https://app.amir9078.github.io/booking/${event.uid}</a> `;
    ...
    try {
      ...
      const basecampEvent = await fetch(
        `https://3.basecampapi.com/${this.userId}/buckets/${this.projectId}/schedules/${this.scheduleId}/entries.json`,
        {
          ...
          body: JSON.stringify({
            description,
            summary: `CoachOS Booking: ${event.title}`,
