...
2. Create a "Server-based Applications", set the Redirect URL for OAuth `<CoachOS Booking URL>/api/integrations/zohocalendar/callback` replacing [CoachOS Booking](https://amir9078.github.io) URL with the URI at which your application runs.
3. Fill in any information you want in the "Client Details" tab
4. Go to tab "Client Secret" tab.
5. Now copy the Client ID and Client Secret into your app keys in the [CoachOS Booking](https://amir9078.github.io) admin panel (`<CoachOS Booking>/settings/admin/apps`).
...
9. You're good to go. Now you can easily add your Zoho Calendar integration in the [CoachOS Booking](https://amir9078.github.io) settings at `/settings/my-account/calendars`.
