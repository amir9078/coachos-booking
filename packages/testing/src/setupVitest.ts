...
import type { CalendarService } from "@coachos/types/Calendar";
...
vi.mock("@coachos/exchangecalendar/lib/CalendarService", () => ({
  default: MockExchangeCalendarService,
}));

vi.mock("@coachos/exchange2013calendar/lib/CalendarService", () => ({
  default: MockExchangeCalendarService,
}));

vi.mock("@coachos/exchange2016calendar/lib/CalendarService", () => ({
  default: MockExchangeCalendarService,
}));
...
function createMockPaymentService(_credentials?: unknown) {
  return {
    async create(
      ...
      const { default: prismaMock } = await import("@coachos/testing/lib/__mocks__/prisma");
    ...
    async afterPayment(
      ...
      const { sendAwaitingPaymentEmailAndSMS } = await import("@coachos/emails/email-manager");
...
vi.mock("@coachos/app-store/stripepayment/index", () => ({
  BuildPaymentService: createMockPaymentService,
}));

vi.mock("@coachos/app-store/paypal/index", () => ({
  BuildPaymentService: createMockPaymentService,
}));

vi.mock("@coachos/app-store/alby/index", () => ({
  BuildPaymentService: createMockPaymentService,
}));

vi.mock("@coachos/app-store/hitpay/index", () => ({
  BuildPaymentService: createMockPaymentService,
}));

vi.mock("@coachos/app-store/btcpayserver/index", () => ({
  BuildPaymentService: createMockPaymentService,
}));

vi.mock("@coachos/app-store/mock-payment-app/index", () => ({
  BuildPaymentService: createMockPaymentService,
}));
...
vi.mock("@coachos/app-store/crm.apps.generated", () => ({
  CrmServiceMap: {
...
if (!process.env.INTEGRATION_TESTS) {
  vi.mock("@coachos/app-store/salesforce/lib/graphql/documents/queries", () => ({
